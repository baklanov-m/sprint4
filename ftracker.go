...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	speedRun := meanSpeed(action, duration)
	return ((runningCaloriesMeanSpeedMultiplier * speedRun * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	meanSpeedRes := meanSpeed(action, duration) * kmhInMsec
	heightMetr := height / cmInM
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedRes, 2)/heightMetr)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
